...
class FerdigstillingService(
    ...
    fun ferdigstillUtenlandskOppgave(
        ...
        requireNotNull(oppgave.dokumentInfoId) {
            "DokumentInfoId må være satt for å kunne ferdigstille oppgave"
        }
        ...
        if (safJournalpostGraphQlClient.erFerdigstilt(journalpost)) {
            log.info(
                "Journalpost med id ${oppgave.journalpostId} er allerede ferdigstilt, sykmeldingId ${oppgave.sykmeldingId}"
            )
    ...
    private fun updateUtenlandskDocumentTitle(
        ...
        securelog.info(
            "documents: ${oppgave.dokumenter.map { it.tittel }} source: ${oppgave.source} sykmeldingId: ${receivedSykmelding.sykmelding.id} "
        )
