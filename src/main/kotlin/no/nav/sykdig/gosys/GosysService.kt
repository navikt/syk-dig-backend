...
class GosysService(
    ...
    fun opprettNasjonalOppgave(papirSmRegistering: PapirSmRegistering): NasjonalOppgaveResponse {
        ...
        if (oppgave.status == "FERDIGSTILT") {
            log.warn("Oppgave med id $oppgaveId er allerede ferdigstilt. Oppretter ny oppgave for sykmeldingId ${papirSmRegistering.sykmeldingId}")
            val opprettOppgave = createOpprettOppgave(papirSmRegistering)
            val opprettetOppgave = oppgaveClient.opprettNasjonalOppgave(opprettOppgave, papirSmRegistering.sykmeldingId)
            return opprettetOppgave
        }
        ...
        val oppdatertOppgave = oppgaveClient.oppdaterNasjonalGosysOppgave(patch, papirSmRegistering.sykmeldingId, oppgaveId.toString(), null)
        return oppdatertOppgave

    }

    private fun createOpprettOppgave(papirSmRegistering: PapirSmRegistering): OpprettNasjonalOppgave {
